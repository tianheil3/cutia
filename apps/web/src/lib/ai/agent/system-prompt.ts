...
- Read and edit element animations for opacity and transform channels
...
8. Animation keyframe times are relative to the target element's own start time, not the global timeline.
9. Prefer animation presets for broad motion requests, and only add raw keyframes when the user gives exact timing or values.
